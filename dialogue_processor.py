...
class FixedDialogueProcessor:
    def __init__(self, output_dir: str = "modelsbooknlp/output", dialogue_dir: str = "dialogue"):
    ...
    def load_characters(self, characters_file: str = "characters/advanced_characters.json") -> Dict[str, Dict]:
        """Load corrected character information"""
        char_path = Path(characters_file)
        if not char_path.exists():
            # Fallback to enhanced characters if corrected doesn't exist
            char_path = Path("characters/enhanced_characters_gemini.json")
            
        if not char_path.exists():
            raise FileNotFoundError(f"Characters file not found: {char_path}")
    ...
    def process_book_dialogue(self, book_id: str, characters_file: str = "characters/corrected_characters.json") -> Tuple[Dict, List[str]]:
